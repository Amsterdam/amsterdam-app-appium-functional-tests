...
class GesturesScreen {
    ...
    static async checkIfDisplayedWithSwipeUp(element, maxScrolls, amount = 0) {
        ...
        if (amount >= maxScrolls) {
            ...
            throw new Error(`The element '${await element}' could not be found or is not visible.`);
        }
    }
  ...
  */
    ...
    */
    static async swipeRightSelector(selector) {
        ...
        this.swipe(
            { x: Math.round(swipeItem.x + (swipeItem.width * 0.20)), y },
            { x: Math.round(swipeItem.width - (swipeItem.width * 0.20)), y },
        );
    }
    ...
    */
    ...
    static async swipeOnPercentageScrollView(selector, from, to) {
        ...
        SCREEN_SIZE = (await driver.getElementRect(elementId)) || SCREEN_SIZE || (await driver.getWindowRect())
        // Get the start position on the screen for the swipe
