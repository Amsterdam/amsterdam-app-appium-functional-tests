...
Given(/ik ben op het afvalwijzer Startscherm/, async () => {
  ...
  await HomeScreen.homeWasteGuideModuleButton.waitForDisplayed({
    timeout: 2000,
  })
...
Given(/^ik heb een adres ingevoerd, bagnummer: (.*)$/, async bagnummer => {
  ...
  await ProfileScreen.bottomSheetSelectAddressButton.waitForDisplayed({
    timeout: 2000,
  })
  ...
  const addressSelector = await ProfileScreen.addressSelector(
    'Balistraat 1, 1 hoog',
  )
  ...
  if (OS === 'iOS') {
    ...
    await expect(attribute).toEqual('Mijn adres, Balistraat 1-1')
  }
  //Android:
  else {
    await expect(
      WasteGuideScreen.wasteGuideRequestLocationButtonText,
    ).toHaveText('Balistraat 1-1')
  }
})

Given(
  /^ik gebruik 'Mijn locatie' met de permissie 'tijdens' bij de afvalwijzer/,
  async () => {
...
Given(
  /^ik gebruik 'Mijn locatie' met de permissie 'altijd vragen' bij de afvalwijzer/,
  async () => {
    await driver.setGeoLocation({
      latitude: 52.363114,
      longitude: 4.907245,
...
Given(
  /^ik geef geen toestemming om 'Mijn locatie' te delen bij de afvalwijzer/,
  async () => {},
)

When(
  /^ik verander het adres naar (.*) met (.*), bagnummer: (.*)$/,
  async (adres, adreslabel, bagnummer) => {
    await WasteGuideScreen.wasteGuideRequestLocationButton.waitForDisplayed({
      timeout: 5000,
    })
    await WasteGuideScreen.wasteGuideRequestLocationButton.click()
    ...
    if (OS === 'iOS') {
      ...
      console.log(attribute)
      await expect(attribute).toEqual(`Mijn adres, ${adres}`)
    }
    //Android:
    else {
      await expect(
...
When(
  /ik voer een adres (.*) in dat geen woonadres is, bagnummer: (.*)/,
  async (adres, bagnummer) => {
    await WasteGuideScreen.wasteGuideRequestLocationButton.waitForDisplayed({
      timeout: 5000,
    })
    await WasteGuideScreen.wasteGuideRequestLocationButton.click()
...
Then(/ik zie het juiste adres in het afvalwijzerscherm/, async () => {
  ...
  if (OS === 'iOS') {
    ...
    await expect(
      WasteGuideScreen.wasteGuideRequestLocationButtonTitle,
    ).toHaveText('Geef uw locatie door')
  }
})
...
Then(
  /ik zie het Afvalwijzer scherm voor adressen die geen woonadres zijn, contract: (.*)/,
  async contract => {
    await expect(
      WasteGuideScreen.wasteGuideRequestLocationButton,
    ).toBeDisplayed()
    await WasteGuideScreen.wasteGuideReportWrongBuildingTypeIntroPhrase.waitForDisplayed(
      {timeout: 5000},
    )
    ...
    if (contract === 'nee') {
      await WasteGuideScreen.wasteGuideSelectContractRadioGroupfalseRadioButton.click()
      await expect(
        WasteGuideScreen.wasteGuideScreenRestafvalTitle,
      ).toBeDisplayed()
    }
    //Selecteer contract == ja, info over restafval mag niet getoond worden
      ...
      await expect(
        WasteGuideScreen.wasteGuideBusinessesInfoTitle,
      ).toBeDisplayed()
      await expect(
        WasteGuideScreen.wasteGuideBusinessesInfoPhrase,
      ).toBeDisplayed()
    }
  },
)
