...
When(/ik klik op het stadsloket/, async () => {
  await gestures.checkProjectDisplayedWithScrollDownSlow(
    ContactScreen.ContactCurrentCityOfficeButton,
    4,
  )
...
Then(/^het juiste stadsloket wordt getoond: (.*)$/, async title => {
  ...
  if (OS === 'iOS') {
    ...
    const currentCityOfficeTitle =
      await ContactScreen.ContactCurrentCityOfficeButton.getAttribute('label')
    await expect(currentCityOfficeTitle).toContain(title)
  } else {
    //define Android
    await driver.pause(2000)
    await gestures.swipeUpSlowFraction()
    await expect(ContactScreen.ContactCurrentCityOfficeButtonTitle).toHaveText(
      title,
    )
  }
})
...
Then(/de bekijk routeknop wordt getoond/, async () => {
  await gestures.checkProjectDisplayedWithScrollDown(
    ContactScreen.contactSeeRouteButton,
    4,
  )
...
Then(/ik zie een maak een afspraakknop/, async () => {
  await gestures.checkProjectDisplayedWithScrollDown(
    ContactScreen.contactMakeAppointmentButton,
    4,
  )
