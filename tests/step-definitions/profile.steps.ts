...
When(/ik voeg mijn huidige locatie toe/, async () => {
  await ProfileScreen.addressAddButton.waitForDisplayed({timeout: 5000})
  await ProfileScreen.addressAddButton.click()
  await ProfileScreen.addressUseLocationButton.waitForDisplayed({
    timeout: 150000,
  })
...
//Then
Then(
  /ik zie mijn profiel met de mogelijkheid om een adres toe te voegen/,
  async () => {
...
Then(/krijg ik de juiste zoekresultaten in het 'Adres' scherm/, async () => {
  await expect(
    ProfileScreen.addressSearchResultWeesperstraat113,
  ).toBeDisplayed()
})
...
Then(/ik zie de privacy en locatie informatie/, async () => {
  ...
  if (OS === 'iOS') {
    const attribute =
      await ProfileScreen.addressPrivacyInfoModalCloseButton.getAttribute(
        'label',
      )
  ...
  //Android:
    ...
    await expect(
      ProfileScreen.addressPrivacyInfoModalCloseButtonLabel,
    ).toHaveText('Oké, ik begrijp het!')
  }
  await expect(ProfileScreen.addressPrivacyInfoTitle).toBeDisplayed()
  await expect(ProfileScreen.addressPrivacyInfoTitle).toHaveText(
    'Zo gebruiken wij uw locatie en adres',
  )
  await expect(
    ProfileScreen.addressPrivacyInfoModalHeaderCloseButton,
  ).toBeDisplayed()
})
...
Then(/er wordt een melding getoond dat er geen suggesties zijn/, async () => {
  await ProfileScreen.addressNoSuggestionsMessage.waitForDisplayed({
    timeout: 30000,
  })
})
