...
Then(/mijn locatie wordt gebruikt voor het tonen van werkzaamheden/, async () => {
    ...
    if (OS === 'iOS') {
        ...
        const locationText = await constructionWorkScreen.constructionWorkRequestLocationButton.getAttribute("content-desc")
    ...
    if (OS === 'iOS') {
        ...
        const locationText = await wasteGuideScreen.wasteGuideRequestLocationButton.getAttribute("content-desc")
        console.log(await locationText)
        //chai.expect(locationText).to.be.oneOf(locationsAndroid)
        chai.expect(locationText).to.be.oneOf(locationsArray)
    }
